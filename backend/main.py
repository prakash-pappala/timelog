...
def delete_category(category_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Delete all sessions linked to this category first to avoid foreign key constraint
    db.query(Session_).filter(Session_.category_id == category_id, Session_.user_id == user.id).delete()
    db.delete(cat)
    db.commit()
